    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($subjects as $subject)
            <a
                ...
                class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 transform hover:-translate-y-1 h-full flex flex-col"
            >
                <div class="p-6 flex-grow flex flex-col">
                    <div class="flex justify-between items-start mb-4">
                        <h3 class="text-xl font-bold text-gray-900">{{ $subject->name }}</h3>
                        <span class="text-sm px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-800">
        ...
        @empty
            <div class="col-span-full bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
                ...
                <p class="text-gray-500 mb-6">{{ __('There are no subjects matching your criteria.') }}</p>
                @if($search)
                    <button 
