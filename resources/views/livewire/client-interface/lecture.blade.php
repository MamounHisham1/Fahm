<div class="space-y-6">
    ...
    @if(isset($upcomingLectures) && count($upcomingLectures) > 0)
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Upcoming Live Lectures') }}</h2>
                ...
                @foreach($upcomingLectures as $lecture)
                    <div class="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $lecture->subject->name ?? 'General' }}</span>
                        </div>
                        <h3 class="text-md font-medium text-gray-900 dark:text-white mb-1">{{ $lecture->title }}</h3>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <div class="w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-300">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                    </svg>
                                </div>
                                <span class="ml-2 text-xs text-gray-600 dark:text-gray-300">{{ $lecture->teacher->name ?? 'Instructor' }}</span>
                            </div>
                            <button 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow duration-200">
                <div class="relative">
                    ...
                    @if($lecture->type === 'live')
                        <div class="absolute top-2 left-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300">
                            <span class="relative flex h-2 w-2 mr-1">
                    ...
                    @elseif($lecture->type === 'upcoming')
                        <div class="absolute top-2 left-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300">
                            {{ __('Upcoming') }}
                        </div>
                    @endif
                </div>
                ...
                <div class="p-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ $lecture->subject->name ?? 'General' }}</span>
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $lecture->created_at ? $lecture->created_at->format('M d, Y') : 'Jan 1, 2023' }}</span>
                    </div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ $lecture->title }}</h3>
                    <p class="text-gray-600 dark:text-gray-300 text-sm mb-4 line-clamp-2">{{ $lecture->description }}</p>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            ...
                            <span class="ml-2 text-sm text-gray-600 dark:text-gray-300">{{ $lecture->teacher->name ?? 'Instructor' }}</span>
                        </div>
                        <button 
        ...
        @empty
            <div class="col-span-full bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('No lectures found') }}</h3>
                <p class="text-gray-500 dark:text-gray-400 mb-6">{{ __('There are no lectures matching your criteria.') }}</p>
        ...
        <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ $selectedLecture->title }}</h3>
                ...
                <div class="p-6">
                    <div class="flex flex-wrap items-center gap-4 mb-4">
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ $selectedLecture->subject->name ?? 'General' }}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Instructor') }}: {{ $selectedLecture->teacher->name ?? 'Instructor' }}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('Duration') }}: {{ $selectedLecture->duration ?? '45:00' }}</span>
                    </div>
                    
                    <div class="prose prose-blue max-w-none dark:prose-invert mb-6">
                        <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-2">{{ __('About this lecture') }}</h4>
                    ...
                    @if(isset($selectedLecture->resources) && count($selectedLecture->resources) > 0)
                        <div class="mt-6">
                            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-3">{{ __('Resources') }}</h4>
                            <div class="space-y-2">
                                @foreach($selectedLecture->resources as $resource)
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $resource->title ?? 'Lecture Notes' }}</p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $resource->type ?? 'PDF' }} • {{ $resource->size ?? '2.4 MB' }}</p>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
        ...
        <div class="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('Join Live Lecture') }}</h3>
                    <button wire:click="closeJoinModal" class="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                    </button>
                ...
                <div class="p-6">
                    <div class="mb-6">
                        <h4 class="font-medium text-gray-900 dark:text-white mb-2">{{ $joiningLecture->title }}</h4>
                        <p class="text-sm text-gray-600 dark:text-gray-300 mb-4">{{ $joiningLecture->description }}</p>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/30 p-4 rounded-lg mb-4">
                            ...
                            <ul class="mt-2 text-sm text-blue-700 dark:text-blue-400 space-y-1 pl-7 list-disc">
                        ...
                        <div class="flex items-center mb-4">
                            ...
                            >
                            <label for="camera" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Enable camera') }}</label>
                        </div>
                        ...
                        <div class="flex items-center">
                            <input 
                                ...
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"
                            >
                            <label for="microphone" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('Enable microphone') }}</label>
                        </div>
                    </div>
