                ...
                <div class="flex items-center gap-3 mb-2">
                    ...
                    @if($assignment->submissions()->where('user_id', Auth::id())->exists())
                        @if($submission->grades->isNotEmpty())
                            <span class="inline-flex items-center gap-1 text-sm font-medium text-blue-800 dark:text-blue-300 bg-blue-100 dark:bg-blue-900/50 px-2.5 py-1 rounded-full">
                        ...
                        @else
                            <span class="inline-flex items-center gap-1 text-sm font-medium text-green-800 dark:text-green-300 bg-green-100 dark:bg-green-900/50 px-2.5 py-1 rounded-full">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                        d="M5 13l4 4L19 7" />
                                </svg>
                    ...
                    @elseif($assignment->due_date && $assignment->due_date->isPast())
                        <span class="inline-flex items-center gap-1 text-sm font-medium text-red-800 dark:text-red-300 bg-red-100 dark:bg-red-900/50 px-2.5 py-1 rounded-full">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                    ...
                    @else
                        <span class="inline-flex items-center gap-1 text-sm font-medium text-amber-800 dark:text-amber-300 bg-amber-100 dark:bg-amber-900/50 px-2.5 py-1 rounded-full">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div
                ...
                @if (!$submission)
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Submit Your Work') }}
                    </h2>
                    <form wire:submit="submitAssignment" class="space-y-4">
                        <div class="space-y-3">
                            <label
                                class="flex items-center p-3 gap-3 rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                                :class="{ 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800': $wire
                                        .type === 'file' }">
                                <input type="radio" name="type" wire:model="type" value="file"
                                    class="text-blue-600 focus:ring-blue-500">
                                <div>
                                    <span
                                        class="block text-sm font-medium text-gray-900 dark:text-white">{{ __('File Upload') }}</span>
                                    <span
                                        class="block text-xs text-gray-500 dark:text-gray-400">{{ __('Upload a document, image, or PDF file') }}</span>
                                </div>
                            </label>
                            ...
                            <label
                                ...
                                <div>
                                    <span
                                        class="block text-sm font-medium text-gray-900 dark:text-white">{{ __('Text Submission') }}</span>
                                    <span
                                        class="block text-xs text-gray-500 dark:text-gray-400">{{ __('Write your submission directly') }}</span>
                                </div>
                            </label>
                        </div>
                ...
                @else
                    ...
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('Your Submission') }}
                    </h2>

                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            <span
                                class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Submitted') }}</span>
                            <span
                                class="text-sm text-gray-500 dark:text-gray-400">{{ $submission->created_at->format('M d, Y - h:i A') }}</span>
                        </div>
